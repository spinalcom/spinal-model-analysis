...
const flattenInput = (input: unknown): unknown[] => {
  if (Array.isArray(input)) return input.flat(Infinity);
  return [input];
};

const isSpinalNodeLike = (value: unknown): value is SpinalNode<any> => {
...
const isNodeInput = (value: unknown): value is NodeInput => {
  return isSpinalNodeLike(value);
};

const getNodeId = (node: NodeInput): string | undefined => {
...
class Algorithm implements IAlgorithm {
  ...
  constructor(
    ...
    outputType: string,
    requiredParams: IAlgorithmParameter[] | 'boolean' | 'number' | 'string',
    run: (input: any | any[], params?: IParameters) => AlgorithmResult
  ) {
    this.name = name;
...
export const FIRST_NODE = new Algorithm(
  'FIRST_NODE',
  'This algorithm returns the first SpinalNode found in the inputs',
  ['SpinalNode'],
  'SpinalNode',
  [],
  (input: unknown[]): NodeInput => {
    const flat = flattenInput(input);
    const foundNode = flat.find(isNodeInput);
    if (!foundNode) throw new Error('No SpinalNode input provided');
    return foundNode;
  }
);

export const NODE_ARRAY = new Algorithm(
  'NODE_ARRAY',
  'This algorithm returns all SpinalNodes found in the inputs',
  ['SpinalNode'],
  'SpinalNode[]',
  [],
  (input: unknown[]): NodeInput[] => {
    ...
    if (foundNodes.length === 0) throw new Error('No SpinalNode input provided');
    return foundNodes;
  }
);

export const UNIQUE_NODE_ARRAY = new Algorithm(
  'UNIQUE_NODE_ARRAY',
  'This algorithm returns all unique SpinalNodes found in the inputs (uniqueness based on node id)',
  ['SpinalNode'],
  'SpinalNode[]',
  [],
  (input: unknown[]): NodeInput[] => {
    ...
    const foundNodes = flat.filter(isNodeInput);
    if (foundNodes.length === 0) throw new Error('No SpinalNode input provided');
    const uniqueNodes = new Map<string, NodeInput>();
    const nodesWithoutId: NodeInput[] = [];
