...
const isNodeInput = (value) => {
    return isSpinalNodeLike(value);
};
...
exports.FIRST_NODE = new Algorithm('FIRST_NODE', 'This algorithm returns the first SpinalNode found in the inputs', ['SpinalNode'], 'SpinalNode', [], (input) => {
    const flat = flattenInput(input);
    const foundNode = flat.find(isNodeInput);
    if (!foundNode)
        throw new Error('No SpinalNode input provided');
    return foundNode;
});
exports.NODE_ARRAY = new Algorithm('NODE_ARRAY', 'This algorithm returns all SpinalNodes found in the inputs', ['SpinalNode'], 'SpinalNode[]', [], (input) => {
    const flat = flattenInput(input);
    const foundNodes = flat.filter(isNodeInput);
    if (foundNodes.length === 0)
        throw new Error('No SpinalNode input provided');
    return foundNodes;
});
exports.UNIQUE_NODE_ARRAY = new Algorithm('UNIQUE_NODE_ARRAY', 'This algorithm returns all unique SpinalNodes found in the inputs (uniqueness based on node id)', ['SpinalNode'], 'SpinalNode[]', [], (input) => {
    const flat = flattenInput(input);
    const foundNodes = flat.filter(isNodeInput);
    if (foundNodes.length === 0)
        throw new Error('No SpinalNode input provided');
...
exports.ALGORITHMS = {
    ...
    FIRST_NODE: exports.FIRST_NODE,
    NODE_ARRAY: exports.NODE_ARRAY,
    UNIQUE_NODE_ARRAY: exports.UNIQUE_NODE_ARRAY,
    EXIT: exports.EXIT,
};
//# sourceMappingURL=algorithms.old.js.map